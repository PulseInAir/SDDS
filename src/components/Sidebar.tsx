...
export function Sidebar() {
  ...
  const updateIndicator = useCallback(() => {
    ...
    if (index > -1 && navRef.current && linkRefs.current[index]) {
      ...
      if (containerRect && activeRect) {
        setStyle({ top: activeRect.top - containerRect.top, height: activeRect.height, opacity: 1 });
      }
    } else {
  ...
  return (
    <nav ref={navRef} className="sdds-sidebar relative flex flex-col h-full py-6 px-4">
      <div 
        className="absolute left-4 right-4 bg-white rounded-xl shadow-lg z-0 transition-all duration-300 ease-out"
        style={{ top: style.top, height: style.height, opacity: style.opacity }}
      />
      {/* Header */}
      <div className="flex items-center gap-3 mb-8 text-white">
        ...
        <span className="font-bold text-lg">SDDS Portal</span>
      </div>
          ...
          return (
            <Link
              ...
              className={`relative z-10 flex items-center gap-3 px-4 py-3 rounded-xl transition-colors duration-200 ${
                isActive ? 'text-blue-700 font-semibold' : 'text-blue-100 hover:text-white'
              }`}
            >
