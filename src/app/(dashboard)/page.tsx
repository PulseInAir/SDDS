...
export default async function DashboardPage({ searchParams }: DashboardProps) {
  ...
  const yetToFileFilings = filings?.filter(f => f.filing_status === 'Yet To File').length || 0;
  ...
  // 3. Fetch Financial Outstanding Totals
  const { data: invoices } = await supabase
    .from('invoices')
    .select('outstanding_amount, settlement_amount')
    .eq('assessment_year', currentAY);

  const totalOutstanding = invoices?.reduce((sum, inv) => sum + Number(inv.outstanding_amount || 0), 0) || 0;
  const totalBilled = invoices?.reduce((sum, inv) => sum + Number(inv.settlement_amount || 0), 0) || 0;
